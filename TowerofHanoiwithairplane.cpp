...
struct aeroplane{
	aeroplane() { normal = Vector3(0.0, 0.0, 1.0); }
...
void initialize_game()
{
	...
	aero.position.x = t_board.x_max;
	aero.position.y = t_board.y_min;
...
void drawaeroplane()
{
	...
	// Back wing (colored blue as per your request)
	glColor3f(0.0, 0.0, 1.0); // Blue color for wings
	glBegin(GL_POLYGON);
	glVertex2f(0.0, 55.0);
	glVertex2f(0.0, 80.0);
	...
	// Left side wing (colored blue as per your request)
	glColor3f(0.0, 0.0, 1.0); // Blue color for wings
	glBegin(GL_POLYGON);
	...
	// Right side wing (colored blue as per your request)
	glColor3f(0.0, 0.0, 1.0); // Blue color for wings
	glBegin(GL_POLYGON);
...
void display_handler() {
	...
	// Calculate the center and base radius of the rods
	double x_center = (t_board.x_max - t_board.x_min) / 2.0;
	double y_center = (t_board.y_max - t_board.y_min) / 2.0;
	double r = t_board.rod_base_rad;
	...
	// Draw the game board and rods
	draw_board_and_rods(t_board);
	draw_discs();

	// Draw the aeroplane if a disc is in motion
	// Draw the aeroplane if a disc is in motion
	if (active_disc.is_in_motion) {
		// Translate the aeroplane to the interpolated position between start and destination
		glPushMatrix();
		glDisable(GL_LIGHTING);

		GLfloat a = aero.position.x; // Previous x translation
		GLfloat c = aero.position.y + 50; // Previous y translation

		// Interpolate the airplane's position between start and destination
		GLfloat interpX = active_disc.start_pos.x + (active_disc.dest_pos.x - active_disc.start_pos.x) * active_disc.u;
		GLfloat interpY = active_disc.start_pos.y + (active_disc.dest_pos.y - active_disc.start_pos.y) * active_disc.u;

		// Define scaling variables
		GLfloat scale = 0.075; // New scaling factor

		// Apply scaling transformation
		glScalef(0.025, scale, 1);

		// Translate the aeroplane to its interpolated position
		glTranslatef(interpX+80, interpY, 5);
...
void render(void)
{
	...
	glColor3f(1, 1, 0);
	drawStrokeText("Towers of Hanoi Simulation", 200, 100, 0);
	drawStrokeText("A Project By : ", 200, 200, 0);
	drawStrokeText("Shivam Verma", 200, 300, 0);
	drawStrokeText("Garv Chandalia", 200, 400, 0);
	drawStrokeText("Aryan Dogra", 200, 500, 0);
	drawStrokeText("Press N to go to the next screen", 200, 600, 0);
	drawStrokeText("Press Q to quit", 200, 700, 0);
...
void anim_handler()
{
	...
	if (active_disc.is_in_motion)
	{
		...
		//	int direction;     // +1 for Left to Right & -1 for Right to left, 0 = stationary
		//};
		while (aero.position.x < active_disc.start_pos.x)
		{
			aero.position.x += 0.05;
			glutPostRedisplay();
			return;	
		}
